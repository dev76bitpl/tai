...
- **Domyślnie jeden job na wszystko, co dzieli checkout i toolchain** — guardy, lint, typecheck, test, build, skan lockfile'a. `npm ci` (albo odpowiednik) wykonuje się raz, nie raz na nogę (~35 s narzutu każda). Kolejność kroków od najtańszego: guardy i lint padają w sekundy, build jest ostatni — przy błędzie w guardach nie płacisz za build. Osobny job zostaw tylko temu, co **nie może** dzielić środowiska: skaner działający we własnym obrazie kontenera.
- **Kiedy zrównoleglać mimo to** — gdy minut jest w nadmiarze (repo publiczne = darmowe, własny runner) albo gdy czas oczekiwania realnie blokuje zespół. Wtedy `strategy.matrix` z `{name, cmd}` + `fail-fast: false` (jeden czerwony leg nie ubija feedbacku z reszty), build osobno bo niesie własny cache. **Nie jest to domyślna rekomendacja** — na darmowym planie z limitem minut rozbicie na joby zjada limit szybciej niż cokolwiek innego, a odkrywa się to dopiero gdy CI staje w połowie miesiąca.
